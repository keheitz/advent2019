...
{
    public class FuelManagementPanel
    {
        private Coordinate port = new Coordinate(0, 0);
        private wire wire1, wire2;

        public int FindManhattenDistanceOfClosestIntersection()
        {
            List<Coordinate> intersections = findIntersections();
            var distances = intersections.ToDictionary(x => x, x => Math.Abs(0 - x.XCoordinate) + Math.Abs(0 - x.YCoordinate));
            var min = distances.OrderBy(kvp => kvp.Value).First();
            return min.Value;
        }

        private List<Coordinate> findIntersections()
        {
            List<Coordinate> intersections = new List<Coordinate>();
            List<string> wire1_locations = wire1.locations.Select(x => x.ToString()).ToList();
            foreach (Coordinate location in wire2.locations)
            {
                ...
                if (wire1_locations.Contains(location.ToString()) && !intersections.Any(x => x.XCoordinate == location.XCoordinate && x.YCoordinate == location.YCoordinate))
                {
                    Coordinate cross_coord = wire1.locations.First(x => x.XCoordinate == location.XCoordinate && x.YCoordinate == location.YCoordinate);
                    intersections.Add(new Coordinate(location.XCoordinate, location.YCoordinate, (location.StepsToReach + cross_coord.StepsToReach)));
                }
            }
    ...
    internal interface IMove
    {
        ...
        void Move(wire wire, Coordinate init_location, int steps);

    }

    internal class UpMover : IMove
    {
        public char MoveInstruction => 'U';

        public void Move(wire wire, Coordinate init_location, int steps)
        {
                Coordinate end_location = new Coordinate(init_location.XCoordinate, init_location.YCoordinate + 1, wire.steps_taken);
                wire.locations.Add(end_location);
        }
    }

    internal class DownMover : IMove
    {
        public char MoveInstruction => 'D';
    ...
    internal class RightMover : IMove
    {
        public char MoveInstruction => 'R';

        public void Move(wire wire, Coordinate init_location, int steps)
        {
                Coordinate end_location = new Coordinate(init_location.XCoordinate + 1, init_location.YCoordinate, wire.steps_taken);
                wire.locations.Add(end_location);
        }
    }

    internal class LeftMover : IMove
    {
        public char MoveInstruction => 'L';
    ...
    internal static class MoverFactory
    {
        ...
        internal static IMove GetMover(char movechar)
        {
            if (movers.Count == 0)
            {
